...
export const SalesList: React.FC = () => {
  ...
  const filteredSales = useMemo(() => {
    return sales.filter((s) => {
      ...
      const medName = md?.name
        ? String(md.name).toLowerCase()
        : String(s.medicine).toLowerCase();
      const matchesSearch =
        !searchTerm || medName.includes(searchTerm.toLowerCase());
  ...
  const handleExport = (format: "csv" | "pdf") => {
    const dataToExport = filteredSales.map((s) => {
      ...
      return {
        ...
        Quantity: s.quantity,
        Total: `Kshs ${(parseFloat(String(s.total_amount)) || 0).toFixed(2)}`,
      };
    });
  ...
  const baseColumns = [
    ...
    },
    {
      ...
      render: (_: any, row: Sale) => {
        const md = (row as any).medicine_detail;
        if (md?.price)
          return `Kshs ${(parseFloat(String(md.price)) || 0).toFixed(2)}`;
        const qty = Number(row.quantity) || 0;
        const total = parseFloat(String(row.total_amount)) || 0;
        const unit = qty > 0 ? total / qty : total;
        return `Kshs ${unit.toFixed(2)}`;
      },
    },
    {
      key: "total",
      header: "Total",
      render: (_: any, row: Sale) =>
        `Kshs ${(parseFloat(String(row.total_amount)) || 0).toFixed(2)}`,
    },
    {
      key: "date",
      header: "Date",
      render: (_: any, row: Sale) =>
        formatDate(row.created_at || (row as any).date || ""),
    },
  ];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Record and review pharmacy sales
          </p>
        </div>
