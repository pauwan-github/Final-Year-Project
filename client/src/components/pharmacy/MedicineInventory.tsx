...
export const MedicineInventory: React.FC = () => {
  ...
  const [editingMedicine, setEditingMedicine] = useState<ApiMedicine | null>(
    null,
  );
  const [selectedMedicine, setSelectedMedicine] = useState<ApiMedicine | null>(
    null,
  );
  ...
  const handleSaleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Failed to fetch updated medicine, refetching list",
          fetchErr,
        );
        fetchMedicines()
          .then((data) => setMedicines(data))
          .catch((err) => console.error("Failed to refetch medicines", err));
      }
      ...
      const message =
        err?.response?.data || err?.message || "Failed to complete sale";
  ...
  const handleExport = (format: "csv" | "pdf") => {
    const dataToExport = filteredMedicines.map((medicine) => ({
      ...
      Description: medicine.description || "",
      Stock: medicine.stock,
      Price: `Kshs ${(parseFloat(medicine.price) || 0).toFixed(2)}`,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, medicine: ApiMedicine) => (
        <div className="flex items-center space-x-2">
          <div>
            <p
              className={`font-medium ${medicine.stock <= 5 ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-white"}`}
            >
              {medicine.stock}
            </p>
          </div>
    ...
    },
    {
      ...
      render: (_: any, medicine: ApiMedicine) => (
        <div>
          <p className="font-medium text-gray-900 dark:text-white">
            Kshs {(parseFloat(medicine.price) || 0).toFixed(2)}
          </p>
        </div>
      ),
    },
        ...
        {/* <Card className="hover:shadow-md transition-shadow">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">Kshs {inventoryStats.totalInventoryValue.toFixed(2)}</p>
            </div>
          </div>
        </Card> */}
        ...
        {/* <Card className="hover:shadow-md transition-shadow">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">Kshs {inventoryStats.totalSalesValue.toFixed(2)}</p>
            </div>
          </div>
        </Card> */}
      </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-medium text-gray-900 dark:text-white">
                  ${parseFloat(selectedMedicine?.price || "0").toFixed(2)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xl font-bold text-blue-600 dark:text-blue-400">
                $
                {(
                  parseFloat(selectedMedicine?.price || "0") *
